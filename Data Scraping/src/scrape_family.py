...
# Lanjutkan setelah klik tombol Family.
...
for button in buttons:
    if button.text.strip() == "Family":
        driver.execute_script("arguments[0].click();", button)

WebDriverWait(driver, 20).until(
    EC.presence_of_element_located((
        By.XPATH,
        "//button[contains(@class, 'c-tvlistingsButton-isActive') and .//span[normalize-space()='Family']]",
    ))
)
...
output_path = DATA_DIR / "family_schedules_raw.json"
with output_path.open("w", encoding="utf-8") as file:
    json.dump(family_schedules, file, indent=2, ensure_ascii=False)

print(f"Jadwal Family berhasil disimpan: {len(family_schedules)}")
