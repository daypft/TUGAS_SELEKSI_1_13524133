...
# Lanjutkan setelah klik tombol Sports
...
for button in buttons:
    if button.text.strip() == "Sports":
        driver.execute_script("arguments[0].click();", button)

WebDriverWait(driver, 20).until(
    EC.presence_of_element_located((
        By.XPATH,
        "//button[contains(@class, 'c-tvlistingsButton-isActive') and .//span[normalize-space()='Sports']]",
    ))
)
...
output_path = DATA_DIR / "sports_schedules_raw.json"
with output_path.open("w", encoding="utf-8") as file:
    json.dump(sports_schedules, file, indent=2, ensure_ascii=False)

print(f"Jadwal Sports berhasil disimpan: {len(sports_schedules)}")
