...
for button in buttons:
    if button.text.strip() == "Movies":
        driver.execute_script("arguments[0].click();", button)

WebDriverWait(driver, 20).until(
    EC.presence_of_element_located((
        By.XPATH,
        "//button[contains(@class, 'c-tvlistingsButton-isActive') and .//span[normalize-space()='Movies']]",
    ))
)
...
output_path = DATA_DIR / "movies_schedules_raw.json"
with output_path.open("w", encoding="utf-8") as file:
    json.dump(movies_schedules, file, indent=2, ensure_ascii=False)

print(f"Jadwal Movies berhasil disimpan: {len(movies_schedules)}")
